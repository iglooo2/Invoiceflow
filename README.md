...
| `AUTH_RESEND_KEY` / `EMAIL_FROM` | optional | **Runtime secret** (contact form + magic link) | Contact form shows an error; magic link hidden; share-link email logs to console |
...
| `AUTH_RESEND_KEY` / `EMAIL_FROM` | Contact form + magic link | **Required Runtime Secret** for `/contact` (Message us). `RESEND_API_KEY` is an alias. `EMAIL_FROM` must be a Resend-verified domain (`InvoiceFlow Studio <noreply@invoiceflowstudio.com>`). Without the key, production submit returns an error instead of hanging. |
...
| Table | Purpose |
|---|---|
| `"StudioSettings"` | 1:1 with `"User"` — name parts, currency, locale, logo data URL, structured address, social URLs, email messages, notification toggles, payment terms, footer, default markup, unique `referralCode` |
...
Default contract details prefill notes on new invoices and estimates. Refer & Earn stores a unique `referralCode` on `StudioSettings` and shows `https://invoiceflowstudio.com/r/{code}`. Earnings are a $0 placeholder — no payouts in this release. QuickBooks is a branded connect page: Upgrade Now for Starter (`/dashboard/billing`), Connect stub for Pro. Live Intuit OAuth is not in this release.
...
5. **Settings → Variables and Secrets** (runtime — *not* only build vars). Add:
   ...
   | `EMAIL_FROM` | Variable (**runtime**) | `InvoiceFlow Studio <noreply@invoiceflowstudio.com>` — domain must be verified in Resend |
